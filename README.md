...
* Character Inspection: right-click a selected character to view its Unicode code point, name, plane, general category, embedded font name (PostScript), family, traits, point size (pt / mm / Q), vertical metrics (ascent / descent / leading), Glyph ID (with CID notation for CJK), and the fill color as written in the PDF.
* Color Picker: View ▸ Color Picker (`Shift + Cmd + K`, or right-click ▸ Color Picker) opens a Digital Color Meter-style floating panel that follows the cursor and reports the color of whatever is drawn under it as the PDF's own values — `DeviceCMYK C 0 M 100 Y 100 K 0`, `DeviceGray 0`, `Separation "DIC 161s*" Tint 50%`, ICCBased, DeviceN — read from the content stream rather than sampled from the screen. Text, fills, strokes (with line width), transparency and blend modes are reported exactly; images, shadings and colored patterns fall back to a rasterized sample marked `(sampled)`. Click to lock the reading, click again to release, and `Esc` to close. Edit ▸ Copy Color as Text (`Shift + Cmd + C`) puts the values on the clipboard; Edit ▸ Copy Color as PDF (`Option + Cmd + C`) copies a 64 pt square filled with the color as vector PDF, color space definition included, so pasting into Illustrator, Keynote or Preview carries the exact `DeviceCMYK` values or the `Separation` spot color rather than a screen approximation. The same two keys do the same jobs in Digital Color Meter.
...
| Color Picker | `Shift + Cmd + K` or right-click ▸ Color Picker (click to lock / unlock, `Esc` to close) |
| Copy Color as Text / as PDF | `Shift + Cmd + C` / `Option + Cmd + C` (while the Color Picker is open) |
